...
arguments
    ...
    for step = 2:steps
        ...
        l_tm1 = exp(1i*kx*h/2).*Uf;
        nl_tm1 = alpha*trapz(ChiS,real(ifft(Uf)))*(1-exp(1i*kx*h/2)).*conv(fft(ChiR),Uf);
        Uf1 = l_tm1 + nl_tm1;

        l_tm2 = exp(1i*kx*h/2).*Uf;
        nl_tm2 = alpha*trapz(ChiS,real(ifft(Uf1)))*(1-exp(1i*kx*h/2)).*conv(fft(ChiR),Uf1);
        Uf2 = l_tm2 + nl_tm2;

        l_tm3 = exp(1i*kx*h/2).*Uf1;
        nl_tm3 = 2*alpha*trapz(ChiS,real(ifft(Uf2)))*(1-exp(1i*kx*h/2)).*conv(fft(ChiR),Uf2) - ...
                 alpha*trapz(ChiS,real(ifft(Uf)))*(1-exp(1i*kx*h/2)).*conv(fft(ChiR),Uf);
        Uf3 = l_tm3 + nl_tm3;

        l_tm = exp(1i*kx*h).*Uf1;
        nl_tm = -((-4-h*1i*kx+exp(1i*kx*h).*(4-3*h*1i*kx-h^2*kx.^kx)) + ...
                (2+h*1i*kx+exp(1i*kx*h).*(-2+h*1i*kx)) + ...
                (-4-3*h*1i*kx+h^2*k.^2+exp(h*1i*kx).*(4+h*1i*k))) / ...
                h^2.*k.^2;

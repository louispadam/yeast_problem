...
arguments (Output)
    ...
    mass = [d0(2:end) + d0(1:end-1), d0(1) + d0(end)] .* ...
           mod([ic(2:end) - ic(1:end-1), ic(1) - ic(end)],1)/2;
    ...
    [~, shift_ind] = min(abs(ic-params.r2));
    ic = mod(ic-params.r2,1);
    N = length(ic);      % number of particles

    %mass = circshift(mass,-(shift_ind-1));
    ...
    data(kk,:) = circshift(density_eul,shift_ind-1);        % save first state
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                data(kk,:) = circshift(density_eul,shift_ind-1);
